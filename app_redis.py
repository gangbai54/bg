...
async def chat_with_ai(query: AIQuery,api_key: str = Depends(verify_api_key)):
    ...
    try:
        ...
        history_str = redis_client.get("chat_history")
        if history_str:
        ...
        if "画" in query.prompt:
            ...
            async def generate_chunks():
                ...
                try:
                    ...
                    current_history.append({"role":"user","content":query.prompt})
                    current_history.append({"role":"assistant","content":full_ai_response})
                    redis_client.set("chat_history",json.dumps(current_history))
                finally:
                    background_db.close()
